...
export function RecapCard({ compData, active, totalBuy, totalMarket, profit, pct, accentCol, redCol, gridCol, tickCol, isDark, theme }) {
  ...
  const stats = [
    { label: t("recap.invested"),    val: `${fmt(totalBuy)} €`,    color: "var(--fg)" },
    { label: t("recap.marketValue"), val: `${fmt(totalMarket)} €`, color: accentCol },
    { label: t("recap.totalProfit"), val: `${profit >= 0 ? "+" : ""}${fmt(profit)} €`, color: profit >= 0 ? accentCol : redCol },
    { label: t("recap.performance"), val: `${pct >= 0 ? "+" : ""}${fmt(pct, 1)} %`,    color: pct >= 0 ? accentCol : redCol },
  ];
    ...
    <div className="bot-grid">
      ...
      <div className="card">
        <div className="card-pad">
          <div className="card-title">{t("recap.profitBySkin")}</div>
          {compData.length > 0 ? (
            <ResponsiveContainer width="100%" height={Math.max(160, compData.length * 42 + 20)}>
              <BarChart data={compData} layout="vertical" margin={{ top:0, right:16, bottom:0, left:0 }}>
                ...
                <YAxis
                  ...
                  tick={{ fontFamily:"'Inter',sans-serif", fontSize:11, fill:tickCol }}
                  tickLine={false} axisLine={false}
                />
                <Tooltip content={<Tip/>} cursor={{ fill:"rgba(128,128,128,0.05)" }}/>
                <ReferenceLine x={0} stroke={isDark ? "#1e1e1e" : "#e4e4e7"}/>
                <Bar dataKey="profit" name={t("recap.totalProfit")} barSize={11} radius={[0,3,3,0]}>
                  {compData.map((e, i) => (
      ...
      <div className="card card-pad">
        <div className="card-title">{t("recap.title")}</div>
              ...
              return (
                <div key={s.id} className="perf-item">
                  ...
                  {s.image && (
                    <img
                      src={s.image}
                      style={{ width:30, height:21, objectFit:"contain", borderRadius:3, marginRight:6 }}
                      alt=""
                    />
                  )}
                  <span className="perf-name">{s.name}</span>
                  <div style={{ textAlign:"right", flexShrink:0 }}>
                    <div className="perf-val" style={{ color: dp >= 0 ? accentCol : redCol }}>
                      {dp >= 0 ? "+" : ""}{dp.toFixed(1)}%
                    </div>
